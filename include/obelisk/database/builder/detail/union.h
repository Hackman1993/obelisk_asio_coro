...
{
    using union_t = std::variant<std::vector<sql_value>, sub_query>;

    class union_ : public base_statement
    {
    public:
        template<typename T>
        requires (std::is_base_of_v<base_statement, T>)
        union_(const T& tar): target_(tar){ }
        union_(const std::initializer_list<sql_value>& tar)
        {
            std::vector<sql_value> result;
