...
namespace obelisk::database::builder::detail
{
    ...
    using countable_t = std::variant<col, raw, sql_value>;

    class count_t : public base_statement{
    public:
        template<typename T>
        ...
        {
            std::string result = std::format("COUNT({}{})",distinct_? "DISTINCT ":"" , std::visit(general_visitor{}, source_));
            return result;
        }
    private:
