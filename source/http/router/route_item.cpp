...
namespace obelisk::http {
    ...
    route_item::route_item(const std::string &path, const std::function<obelisk::task<std::unique_ptr<http_response>>(http_request_wrapper &)> &handler): handler_(handler){
    ...
    obelisk::task<std::unique_ptr<http_response>> route_item::handle(http_request_wrapper &request) {
        if(handler_)
