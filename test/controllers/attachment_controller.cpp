...
boost::cobalt::task<std::unique_ptr<obelisk::http::http_response>> upload(obelisk::http::http_request_wrapper&request) {
    ...
    auto conn = obelisk::database::connection_manager::get_connection<
    ...
    collection.insert_one(make_document(
        ...
        kvp("reference", array{}),
        kvp("file_size", static_cast<int64_t>(std::filesystem::file_size(attachment->temp_path_))),
        kvp("deleted_at", bsoncxx::types::b_date(std::chrono::system_clock::now() + std::chrono::days(1))),
        kvp("uploader", bsoncxx::oid(std::any_cast<const user_data&>(request.additional_data()["__user_info"]).user_id)),
        kvp("extension", extension),
        kvp("origin_name", attachment->filename_)
    ));
