...
namespace module::default_
{
    ...
    public:
        static boost::asio::awaitable<std::unique_ptr<obelisk::http::http_response>> backend_view(obelisk::http::http_request_wrapper&request)
        {
            ...
            // if (utils::can("permission.organization.view", admin_id, org_id, org_id))
            // else
            // {
            //     query.where({
            //         {col("so.id"), org_id},
        ...
        static boost::asio::awaitable<std::unique_ptr<obelisk::http::http_response>> backend_create(obelisk::http::http_request_wrapper&request)
        {
            ...
            auto check_query = obelisk::database::db::select({col("id")}).from({"sys_organizations"}).where({
                {col("id"), parent_id},
            ...
            std::vector<std::pair<col, obelisk::database::core::sql_value>> values{{"name", request.params()["name"].get<std::string>()}};
            ...
            co_await obelisk::database::db::transaction([&values, parent_id](auto connection)->boost::asio::awaitable<void>
            {
                ...
                co_await obelisk::database::db::insert("sys_organization_hierarchy", {"fn_ancestor_id", "fn_descendant_id", "path_length"})
                .values({[parent_id, org_id](query& query){
                    query.select({col("fn_ancestor_id"), org_id, obelisk::database::builder::detail::raw("path_length+1")})
                    .from({"sys_organization_hierarchy"}).where({
                        {"fn_descendant_id", parent_id},
        ...
        static boost::asio::awaitable<std::unique_ptr<obelisk::http::http_response>> backend_update(obelisk::http::http_request_wrapper&request)
        {
            ...
            try_emplace<std::string>("emergency_phone", values, params);
            try_emplace<std::string>("address", values, params);
            co_await obelisk::database::db::update({"sys_organizations"}).set(values).where({
                {col("id"), target_id}
            }).get();
            co_return utils::json_response(nullptr);
        }
        ...
        static void try_emplace(const std::string& key, std::vector<std::pair<obelisk::database::builder::detail::col, obelisk::database::core::sql_value>>& values, nlohmann::json params)
        {
